...
CONFIRM_TEXTS = ["Delete", "حذف", "Yes", "نعم", "Confirm", "تأكيد"]

async def _eval_json(page, script: str):
    try:
        raw = await page.evaluate(script)  # script returns JSON.stringify(...)
...
#     js = r"""
#     (() => {
#       const labels = ["Delete", "حذف", "Yes", "نعم", "Confirm", "تأكيد"];
...
#         if (v && labels.some(t => v.includes(t))) {
#           n.click();
#           return true;
#         }
#       }
#       // Danger-styled fallback (sweetalert / bootstrap)
#       const danger = document.querySelector(".swal2-confirm, button.btn-danger, a.btn-danger, .modal-footer .btn-danger");
...
#         if clicked:
#             log("Delete confirmation clicked.", "OK")
...
async def delete_latest_asset(report_id: str, page) -> bool:
    """
    Assumes the just-created asset is on the last page.
    1) Ensure last main page + last sub page
    2) Read #m-table last row macro id
    3) Open /report/macro/{id}/delete
    4) Confirm (Arabic/English) if prompted
    """
    try:
    ...
    # Scroll a bit to encourage lazy content to render
    try:
    ...
    # await _confirm_delete_if_prompted(del_page)
